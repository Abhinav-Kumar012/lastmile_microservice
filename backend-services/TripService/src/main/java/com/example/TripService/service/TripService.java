...
public class TripService {
  ...
  public void listen(MatchingEvent matchingEvent, Acknowledgment ack) {
    ...
    Trip t =
        new Trip(
            matchingEvent.getRiderId(),
            matchingEvent.getDriverId(),
            matchingEvent.getArrivalId(),
            matchingEvent.getArrivalstationname());
    Trip databaseTrip = tripRepository.save(t);
    kafkaTemplate.send(
        "rider-service", new UpdateStatusEvent(matchingEvent.getArrivalId(), "SCHEDULED"));
    kafkaTemplate.send(
        "notification-service",
        new NotifyPartiesEvent(
            databaseTrip.getRiderId(),
            databaseTrip.getDriverId(),
            databaseTrip.getTripId(),
            matchingEvent.getArrivalId(),
            matchingEvent.getArrivalstationname(),
            matchingEvent.getVehiclenumber()));
